    ...
    <DropdownMenuPrimitive.Root>
      <DropdownMenuPrimitive.Trigger asChild>
        <button
          ...
          className="flex h-8 cursor-pointer items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] px-3 text-xs font-medium text-[#E9D5FF] backdrop-blur-xl transition-colors hover:border-[#7C3AED]/50 hover:bg-white/[0.08]"
        >
        ...
        >
          {MEMORY_MODES.map((mode) => (
            <DropdownMenuPrimitive.Item
              ...
              className={cn(
                "flex cursor-pointer items-center gap-2.5 rounded-xl px-3 py-2 text-sm outline-none select-none data-[highlighted]:bg-[#7C3AED]/20",
                value === mode.id && "bg-[#7C3AED]/15 text-white"
              )}
            >
