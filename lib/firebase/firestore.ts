...
export async function createChat(userId: string, opts?: { title?: string }): Promise<ChatDoc> {
  const id = createId();
...
export async function deleteMessage(chatId: string, messageId: string) {
  await deleteDoc(doc(messagesRef(chatId), messageId));
  const chatRef = doc(db, "chats", chatId);
  const chatSnap = await getDoc(chatRef);
  const current = (chatSnap.data()?.messageCount as number) ?? 0;
  await updateDoc(chatRef, {
    messageCount: Math.max(0, current - 1),
    updatedAt: serverTimestamp(),
  });
}
